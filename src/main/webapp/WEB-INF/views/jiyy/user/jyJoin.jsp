...
window.onload = function() {
    ...
    if (params.has('redirected')) {
        alert('로그인 후 이용 가능합니다. 회원가입을 해주세요.');
    }
};
	...
	$(function() {
		$('#joinBtn').on('click', function() {
			...
			$.ajax({
				...
				type : 'post',
				// data: formData,
				data : $("form[name=frm]").serialize(), //데이터를 직렬화시켜줌 -> json형태로 만들어줌
				//contentType: false,
				// processData: false,
		...
		$('#email').on('change', function() {
			...
			if (optionVal !== '')
				$('#userDomail').val(optionVal).attr('readonly', true);
			else
				$('#userDomail').val('').focus().attr('readonly', false);
		});
	});
	...
	function validationCheck() {
		...
		$('#frm').find('input').each(
				function(index, item) {
					if ($(this).val() === null || $.trim($(this).val()) === ''
							|| $(this).val() === 'undefined'
							|| $(this).val().length === 0) {
						//위의 if문의 공백이 발견되면 다음 if문 진입-> id값이 emali이 아니므로 거짓이 되고 else문으로 진입함.!
						if ($(this).attr('id') === 'userDomail'
								&& $('[id=email] :selected').val() !== '') {
							isRight = true;
						} else {
		...
		//.test()는 자바스크립트의 정규표현식을 사용하여 문자열이 특정 패턴과 일치하는지 확인하는 메서드.
		...
		if (!regEmail
				.test($("#userEmail").val() + '@' + $("#userDomail").val())) {
...
<body>
	<div class="wrap">
		...
		<input type="hidden" name="hiddenPw" id="hiddenPw" /> --%>
				<div>
					...
					<div>
						<input type="text" id="userId" name="userId" placeholder="ID 입력"
							data-name="아이디" maxlength="20W"/>
						<button type="button" onclick="doubleCheckId();">중복 확인</button>
					</div>
				</div>
				<div>
					<label for="userPw">비밀번호 <span>*</span></label> <input
						type="password" id="userPw" name="userPw" placeholder="비밀번호 입력"
						data-name="비밀번호" maxlength="16" autoComplete="off" />
				</div>
				<div>
					<label for="userPwCheck">비밀번호 확인 <span>*</span></label> <input
						type="password" id="userPwCheck" name="userPwCheck"
						placeholder="비밀번호 입력" data-name="비밀번호 확인" maxlength="16"
						autoComplete="off" />
				</div>
				<div>
					<label for="userNm">이름 <span>*</span></label> <input type="text"
						id="userNm" name="userNm" placeholder="이름 입력" data-name="이름"
						maxlength="25" />
				</div>
				<div>
					...
					<div>
						<input type="text" id="userEmail" name="userEmail"
							placeholder="이메일 입력" data-name="이메일" maxlength="15" /> <span>@</span>
						<input type="text" id="userDomail" name="userDomail"
							placeholder="직접 입력" data-name="도메인" maxlength="15" /> <select
							id="email">
				...
				<div>
					<label for="userPhone">전화번호 <span>*</span></label> <input
						type="text" id="userPhone" name="userPhone"
						placeholder="휴대폰 번호 입력 ('-' 제외 11자리 입력)" data-name="전화번호"
						maxlength="20"/>
				</div>
				<div>
					<label for="userZipCode">주소 <span>*</span></label> <input
						type="text" class="userZipCode" id="userZipCode"
						name="userZipCode" placeholder="클릭하세요"
						onclick="openDaumPostcode('userZipCode','userFirstAddr','userSecondAddr'); return false;"
						readonly="true" data-name="우편 번호" /><br> <input type="text"
						class="userAddr" id="userFirstAddr" name="userFirstAddr"
						placeholder="주소" readonly="readonly" data-name="주소" /><br> <input
						type="text" id="userSecondAddr" name="userSecondAddr"
						placeholder="나머지 주소" data-name="나머지 주소" maxlength="30" />
					<div id="layer"
	...
	<script>
		...
		function initLayerPosition() {
			...
			element_layer.style.left = (((window.innerWidth || document.documentElement.clientWidth) - width) / 16 - borderWidth)
					+ 'px';
