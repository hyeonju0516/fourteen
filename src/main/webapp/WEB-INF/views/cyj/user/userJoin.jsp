		...
		if(confirm("회원가입을 하시겠습니까?") === true) {
			...
			$.ajax({
				...
				success : function(data) {
					successAlert('', '정상적으로 회원가입이되었습니다.\\n로그인 페이지로 이동합니다.');
					location.href = '/cyj/cyjLoginPage';
				},
				error: function(e) {
					console.log('>>>>>>> :: ', e)
				},
...
function doubleCheckId(check) {
	if($('#userId').val() === undefined || $('#userId').val() === '' || $('#userId').val().length === 0 || $.trim($('#userId').val()) === '') {
		$('#userId').focus();
		errorAlert('', 'ID를 입력하세요');
		return false;
	}

	$.ajax({
		url : '/cyj/user/doubleCheckId',
		...
		success : function(data) {
			if(data.result) {
				errorAlert('', '이미 가입된 ID입니다.')
				idCheck = false;
			} else {
				if(!check) successAlert('', '사용 가능한 ID입니다.')
				idCheck = true;
			}
		},
		error: function(e) {
...
function validationCheck() {
	...
	if(!regId.test($('#userId').val())) {
		errorAlert('', '잘못된 아이디 형식입니다. (4~20자 이내 영문, 숫자를 사용한 아이디)');
		$('#userId').focus();
		return false;
	}
	...
	if(!regHan.test($('#userNm').val())) {
		$('#userNm').focus();
		errorAlert('', '2~10자 이내의 한글만 입력 가능합니다.');
		return false;
	}

	if(!regpw.test($('#userPw').val())) {
		$('#userPw').focus();
		errorAlert('', '비밀번호는 영문자+숫자+특수문자 조합으로 8~16자리 사용해야 합니다.');
		return false;
	}
	...
	if(!regPhone.test($('#userPhone').val())) {
		$('#userPhone').focus();
		errorAlert('', '전화번호는 숫자만 입력할 수 있으며, 올바른 형식이어야 합니다.');
		return false;
	}
...
function initLayerPosition() {
	...
	let height = 460;			//우편번호서비스가 들어갈 element의 height
