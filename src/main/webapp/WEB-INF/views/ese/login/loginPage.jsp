...
$(function(){
	if('${param.MOVE_ERR}' === 'ERRORCONTECT') {
		...
		top.location='/ese/login/loginPage';
	}
	...
	$("#otpInsertBtn").on("click", function(){
		...
		var url = '/ese/login/googleOtpPop';
	...
	$('#phoneInsertBtn').on('click', function(){
		...
		$.ajax({
			type : "post",
			url:"==========================/cyj/send/phoneSend?phoneNumber=" + phoneNumber,
			beforeSend : function() {
				lodingBarStart();
			},
			success : function(result) {
	...
	$(document).on('click', '#phoneCheckedBtn', function(){
		...
		phoneNumber = phoneNumber.substring(0, 3) + "-" + phoneNumber.substring(3, 7) + "-" + phoneNumber.substring(7, 11);
		$.ajax({
			url : '=========================/cyj/send/certificationCodeCheck',
			type : "post",
	...
	<div class="login>
		...
		<form name="frm">
			...
			<div>
				...
				<input type="password" id="userPw" name="userPw" maxlength="16" autocomplete="off" required />
			</div>
			<div>
				<label for="otpNumber">OTP</label>
				<input type="text" id="otpNumber" name="otpNumber" maxlength="6" onkeyup="checkOtp(this);" required />
			</div>
		...
		<ul class="find_wrap">
			<li><a href="/ese/login/findIdForm">아이디 찾기</a></li>
			<li><a href="/ese/login/resetPasswordPage">비밀번호 찾기</a></li>
			<li><a href="/ese/user/joinPage">회원가입</a></li>
		</ul>
	</div>
</div>
