...
$(document).ready(function(){
	$("#otpBtn").click(function(){
		...
		let encryptedPw = rsa.encrypt($('#userPw').val());
		$('#hiddenPw').val(encryptedPw);

		$.ajax({
			url: "/ese/login/otpCreate",
			method: "GET",
			data: {
					...
					success: function(result) {
						if (result.resultCode != 'CM0000') {
							errorAlert('', result.resultMsg);
								...
								$('.otpSpan').remove();
								$('.qrCodeDiv').append('<p style="color:red;">아래 QR바코드를 구글 OTP앱에서 바코드 스캔으로 등록해주세요</p><img alt="qr코드" src="' + result.qrCode + '" />');
