...
$(function(){
    $('#joinBtn').on('click', function(){
        ...
        if(!idDoubleCheck(true)) return false;

        var formData = new FormData($('form')[0]);

        $.ajax({
            url : '/ysh/user/userJoin',
            type: 'post',
            // data: formData,
...
function validationCheck() {
	...
	if(!isRight) return false;		// 입력해야할 항목이 남았다면 => isRight=false => 아래 정규식 구문을 거치면 안된다.
	...
	<main>
		...
		<input type="hidden" name="hiddenPw" id="hiddenPw" /> --%>
			<div>
				...
				<div>
					<input type="text" id="userId" name="userId" placeholder="ID 입력" data-name="아이디" maxlength="20" />
					<button type="button" onclick="idDoubleCheck();">중복 확인</button>
				</div>
			</div>
			<div>
